...
基于 ReactFlow 的流程图编辑器，支持 BPMN 2.0 标准，包含 IF 节点、排他网关、包容网关以及 n8n 风格的规则配置系统。

## 功能特性

- 🎨 拖拽式节点创建（开始、结束、任务、IF、排他网关、包容网关）
- 🔀 IF 节点：简单的 true/false 二元条件判断
- 🔗 可视化连接线配置与条件表达式
...
1. **拖拽 IF 节点**到画布
2. **点击节点**打开属性面板
3. **配置条件规则**（支持多条件 AND/OR 组合）
4. **连接 TRUE 输出点**（上方）到条件满足的分支
5. **连接 FALSE 输出点**（下方）到条件不满足的分支

### 与网关的区别

| 特性 | IF 节点 | 排他网关 | 包容网关 |
|------|---------|----------|----------|
| 出口数量 | 固定 2 个 (TRUE/FALSE) | 多个 | 多个 |
| 条件配置 | 节点内配置单一条件 | 每条边配置条件 | 每条边配置条件 |
| 执行逻辑 | 二元判断 | 按优先级单选 | 多选 |
| 适用场景 | 简单的是/否判断 | 多路分支选择 | 并行分支执行 |

### BPMN 导出

IF 节点在导出 BPMN 时会被转换为 `exclusiveGateway`：
- TRUE 分支：导出条件表达式
- FALSE 分支：自动导出条件表达式的否定（`not(...)`）
